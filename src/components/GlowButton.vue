...
const handleKeyCapture = (e: KeyboardEvent) => {
    ...
    if (
        keys.length >= 2 &&
        !["Control", "Alt", "Shift", "Super", "Meta"].includes(
            keys[keys.length - 1],
        )
    ) {
                ...
                <div
                    ...
                    :class="[
                        button?.activeInstances && button.activeInstances > 0
                            ? button.isPaused
                                ? 'bg-orange-500/10'
                                : 'bg-primary/5 shadow-[0_0_20px_rgba(59,130,246,0.1)] scale-[1.01]'
                            : '',
                    ]"
                >
                    <!-- Controls Overlay -->
                    <div
                        v-if="
                            ...
                            button.activeInstances > 0
                        "
                        class="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-20"
                    >
                        <button
                            @click.stop="handleTogglePause"
                            class="p-1.5 hover:bg-accent rounded-full bg-background/80 shadow-sm border border-border"
                        >
                        ...
                        <button
                            ...
                            class="p-1.5 hover:bg-accent rounded-full bg-background/80 shadow-sm border border-border text-destructive"
                        >
                    ...
                    >
                        <span
                            class="text-[9px] font-mono bg-muted/90 px-1.5 py-0.5 rounded border border-border/50 shadow-sm text-primary"
                        >
                            {{ button.shortcut }}
                        </span>
                    </div>
                    ...
                    >
                        <div v-if="button?.path" class="text-center p-4 w-full">
                            <div
                                class="font-bold truncate w-full text-lg mb-1 group-hover:text-primary transition-colors text-primary"
                            >
                                {{ button.name }}
                            </div>
                            <div
                                class="text-xs text-muted-foreground mt-1 flex flex-col items-center gap-1"
                            >
                                <div
                                    class="flex items-center gap-2 text-slate-700 dark:text-slate-300"
                                >
                                    <span
                                        v-if="button.activeInstances > 0"
                                        class="text-[10px] font-mono bg-primary px-2 py-0.5 rounded text-primary-foreground animate-pulse"
                                    >
                                        {{ button.activeInstances }}x
                                    </span>
                                    <span class="font-medium"
                                        >{{
                                            Math.round(button.volume * 100)
                                        }}%</span
                                    >
                                </div>
                            </div>
                        ...
                        <div
                            v-else
                            class="text-muted-foreground text-sm flex flex-col items-center gap-3 opacity-40 group-hover:opacity-100 transition-opacity"
                        >
                            <div
                                class="w-10 h-10 rounded-full bg-muted flex items-center justify-center border border-white/10 shadow-inner"
                            >
                    ...
                    >
                        <div
                            class="h-full bg-primary transition-[width] duration-150 ease-linear shadow-[0_0_10px_rgba(59,130,246,0.5)]"
        ...
        <ContextMenuContent class="w-56">
            ...
            <ContextMenuItem @select="openShortcutDialog" class="gap-2">
                ...
                <span v-if="button?.shortcut"
                    >Set Shortcut ({{ button.shortcut }})</span
                >
                <span v-else>Set Shortcut...</span>
            </ContextMenuItem>
            <ContextMenuItem
                v-if="button?.shortcut"
                @select="handleClearShortcut"
            ...
            <div class="p-2">
                <label
                    class="text-[10px] mb-2 block font-bold uppercase tracking-wider opacity-50"
                    >Button Volume</label
                >
                <Slider
    ...
    <AlertDialog v-model:open="isRenameOpen">
        <AlertDialogContent>
            ...
            <AlertDialogFooter>
                ...
                <AlertDialogAction @click="handleRename"
    ...
    <AlertDialog v-model:open="isShortcutOpen">
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    Press a key combination (e.g., Ctrl+1, Alt+F1). The shortcut
                    will work even when the app is minimized.
                </AlertDialogDescription>
            </AlertDialogHeader>
            <div class="py-4">
                <div
                    class="h-16 flex items-center justify-center border-2 border-dashed rounded-lg bg-muted/50"
                >
                    <span
            ...
            <AlertDialogFooter>
                ...
                <AlertDialogAction
                    ...
                    :class="{
                        'opacity-50 cursor-not-allowed': !capturedShortcut,
                    }"
                >
                    Save Shortcut
                </AlertDialogAction>
