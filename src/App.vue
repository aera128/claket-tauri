    ...
    listen("file-dropped", (event: any) => {
        ...
        if (gridRef.value) {
            ...
            if (
                ...
                const rowHeight =
                    rect.height / Math.ceil(store.itemsPerPage / 4);
...
const handleSeekEnd = async (instanceId: number) => {
    ...
    if (item && localSeekValue.value !== null) {
        ...
        await invoke("seek_instance", {
            instanceId,
            positionMs: Math.round(finalPos),
        });
...
const getVUColor = (index: number, total: number) => {
    ...
    // Green: 0% to 70%
    if (position > 0.9) return "text-red-500 bg-red-500";
    if (position > 0.7) return "text-yellow-400 bg-yellow-400";
...
<template>
    <AuroraBackground
        class="min-h-screen"
        :reduced-motion="store.reducedMotion"
        :accent-color="store.accentColor"
    >
        <TitleBar />
        <div class="w-full h-screen flex flex-col relative overflow-hidden">
            <header
                class="w-full p-6 flex justify-center fixed top-2 left-0 right-0 z-50 pointer-events-none"
            >
                <div
                    class="island-navbar pointer-events-auto flex items-center justify-between gap-6 px-6 h-12 rounded-full border border-border bg-card/60 shadow-2xl min-w-[500px] max-w-[95%] transition-all duration-500 hover:scale-[1.01] hover:border-primary/20 text-slate-950 dark:text-white"
                    :class="{ 'backdrop-blur-2xl': !store.reducedMotion }"
                >
                    <h1
                        class="text-base font-bold tracking-tight flex items-center gap-2 text-nowrap"
                    >
                        <img
                            :src="logo"
                            class="w-6 h-6 object-contain brightness-110"
                            alt="Claket Logo"
                        />
                        Claket
                    </h1>
            ...
            <ScrollArea
                class="flex-1 w-full transition-all duration-500 h-[1px]"
                :class="{
                    'blur-md scale-95 opacity-50': store.isQueueExpanded,
                }"
            >
                <div
                    class="max-w-4xl mx-auto flex flex-col gap-6 p-6 pt-24 pb-32"
                >
                    <div class="flex items-center justify-between px-2">
                        <h2
                            class="text-xl font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2 group cursor-pointer"
                            @click="openPageRename"
                        >
                            <span
                                class="w-2 h-2 rounded-full bg-primary animate-pulse"
                            ></span>
                            {{
                                store.pageNames[store.currentPage] ||
                        ...
                        <div
                            class="flex items-center gap-2 bg-card/40 p-1 rounded-full border border-border"
                        >
                            <Button
                                variant="ghost"
                                size="icon"
                                class="rounded-full h-8 w-8 text-primary"
                                ...
                                <AlertDialogContent>
                                    <AlertDialogHeader>
                                        ...
                                        <AlertDialogDescription>
                                            ...
                                            current page and all its configured
                                            sound buttons. This action cannot be
                                            undone.
                                        </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
            ...
            <Transition name="slide-up">
                <div
                    v-if="store.isQueueExpanded"
                    class="fixed inset-0 z-30 bg-background/60 flex flex-col pt-20"
                    :class="{ 'backdrop-blur-xl': !store.reducedMotion }"
                >
                    <div
                        class="flex-1 overflow-y-auto p-8 max-w-4xl mx-auto w-full text-slate-950 dark:text-white"
                    >
                        <div class="flex items-center justify-between mb-8">
                            ...
                            <Button
                                variant="ghost"
                                size="icon"
                                @click="store.isQueueExpanded = false"
                            >
                                <ChevronDown :size="32" />
                            </Button>
                        </div>
                        ...
                        <div
                            v-if="store.queueList.length === 0"
                            class="flex flex-col items-center justify-center h-64 text-muted-foreground gap-4"
                        >
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div
                                                class="text-xs text-muted-foreground font-mono uppercase tracking-widest text-nowrap text-[10px]"
                                            >
                                                ID: {{ item.instance_id }} •
                                                {{
                                                    formatDuration(
                                ...
                                <div class="flex items-center gap-4 px-2">
                                    <Slider
                                        ...
                                        @update:modelValue="
                                            (v) =>
                                                handleSeek(item.instance_id, v)
                                        "
                                        @pointerdown.capture="
                                            handleSeekStart(item.instance_id)
                                        "
                                        @pointerup="
                                            handleSeekEnd(item.instance_id)
                                        "
                                    />
                                </div>
                            </div>
            ...
            <footer
                class="fixed bottom-0 left-0 right-0 border-t bg-card/80 z-40 text-slate-950 dark:text-white"
                :class="{ 'backdrop-blur-xl': !store.reducedMotion }"
            >
                <div
                    class="w-full h-1 bg-accent/30 relative cursor-pointer group"
                >
                    <Slider
                        ...
                        :model-value="
                            store.seekingInstanceId ===
                                store.latestProgress.instance_id &&
                            localSeekValue !== null
                                ? [localSeekValue]
                                : [store.latestProgress.position_ms]
                        "
                        :max="store.latestProgress.duration_ms"
                        :step="1"
                        @update:modelValue="
                            (v) =>
                                handleSeek(store.latestProgress!.instance_id, v)
                        "
                        @pointerdown.capture="
                            handleSeekStart(store.latestProgress!.instance_id)
                        "
                        @pointerup="
                            handleSeekEnd(store.latestProgress!.instance_id)
                        "
                    />
                    <div
                        v-if="store.latestProgress"
                        ...
                        :style="{
                            width: `${((store.seekingInstanceId === store.latestProgress.instance_id && localSeekValue !== null ? localSeekValue : store.latestProgress.position_ms) / store.latestProgress.duration_ms) * 100}%`,
                        }"
                    ></div>
                </div>

                <div
                    class="max-w-[1400px] mx-auto px-6 h-20 flex items-center justify-between gap-4"
                >
                    <div class="flex items-center gap-4 w-[30%] min-w-0">
                        <div
                            class="w-12 h-12 bg-accent/50 rounded-md flex items-center justify-center flex-shrink-0 overflow-hidden relative group cursor-pointer"
                            @click="
                                store.isQueueExpanded = !store.isQueueExpanded
                            "
                        >
                            <Music
                                v-if="!store.latestProgress"
                            ...
                            >
                                <Music
                                    ...
                                    :class="{
                                        'animate-pulse':
                                            !store.latestProgress.is_paused,
                                    }"
                                />
                            </div>
                            <div
                        ...
                        <div class="min-w-0 flex-1">
                            <div
                                class="text-sm font-semibold truncate cursor-pointer hover:underline text-slate-950 dark:text-white"
                                @click="
                                    store.isQueueExpanded =
                                        !store.isQueueExpanded
                                "
                            >
                                {{
                                    store.latestProgress?.name ||
                                    "No sound playing"
                                }}
                            </div>
                            <div
                            ...
                            >
                                {{
                                    store.latestProgress
                                        ? `${formatDuration(store.latestProgress.position_ms)} / ${formatDuration(store.latestProgress.duration_ms)}`
                                        : "Ready"
                                }}
                            </div>
                        </div>
                    ...
                    <div
                        class="flex items-center gap-4 w-[30%] justify-end relative h-full"
                    >
                        <div class="relative flex items-center h-full">
                            <div
                                ...
                                :class="
                                    store.isVUMeterExpanded
                                        ? 'h-56 w-8 -translate-y-24 shadow-2xl bg-card/95 backdrop-blur-md ring-1 ring-white/10'
                                        : 'h-10 w-4'
                                "
                                @click="
                                    store.isVUMeterExpanded =
                            ...
                            >
                                <div
                                    v-for="i in store.isVUMeterExpanded
                                        ? 48
                                        : 12"
                                    :key="i"
                                    ...
                                    :class="
                                        getVUColor(
                                            i,
                                            store.isVUMeterExpanded ? 48 : 12,
                                        )
                                    "
                                    :style="{
                                        opacity: isSegmentActive(
                                            i,
                                            store.isVUMeterExpanded ? 48 : 12,
                                        )
                                            ? 1
                                ...
                                <div
                                    v-if="store.isVUMeterExpanded"
                                    class="absolute -left-7 inset-y-0 flex flex-col justify-between text-[7px] font-mono text-muted-foreground py-1 pointer-events-none items-end pr-1"
                                >
                        ...
                        <div
                            class="flex items-center gap-2 group/vol min-w-[150px]"
                        >
                            ...
                            <span
                                class="text-[10px] font-mono w-8 text-right text-slate-900 dark:text-slate-100"
                                >{{
        ...
        <AlertDialogContent>
            ...
            <div class="py-4">
                <Input
                    ...
                    @keyup.enter="handlePageRename"
                />
            </div>
            <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction @click="handlePageRename"
